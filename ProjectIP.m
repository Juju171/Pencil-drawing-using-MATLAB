...
%% Imports
% Image
img_filename = 'man'; % Only change this value (names in the inputs folder)
ex_img = imread(['./inputs/', img_filename, '.jpg']);
[height, width] = size(ex_img); % Image dimensions

% Pencil
pencil_nbr = '5'; % Only change this value (0 to 5 (1 is bad))
pencil_filename = ['pencil', pencil_nbr]; 
pencil_texture = imread(['./pencils/', pencil_filename, '.jpg']);

%% Parameters
kernel_size = round(height/50);     % size of the line segment kernel (usually 1/50 of the height of the original image)
stroke_width = 1;                   % thickness of the strokes in the Stroke Map (1, 2, 3)
...
w_group = 2;                        % 3 possible weight groups (0, 1, 2) for the histogram distribution, according to the paper (brighter to darker)
stroke_darkness = 1;                % 1 is the same, up is darker
tone_darkness = 2;                  % 1 is the same, up is darker
...
% figure;
% for i = 0:3
%     ex_img_stroke_map_i = gen_stroke_map(img, round(height / ((i + 1) * mult)), stroke_width, num_of_directions, smooth_kernel);
...
% for i = 0:2
%     ex_img_stroke_map_i = gen_stroke_map(img, kernel_size, i, num_of_directions, smooth_kernel);
...
% figure;
% for i = 0:3
%     ex_img_stroke_map_i = gen_stroke_map(img, kernel_size, stroke_width, (i+1)*4, smooth_kernel);
%     subplot(2,2,i+1); imshow(ex_img_stroke_map_i); title(['Num of direction value ', num2str((i+1)*4)]); axis off;
% 
%     % Save results
...
% for i = 0:2
%     ex_im_pen_i = gen_pencil_drawing(ex_img, kernel_size , stroke_width,
%     num_of_directions, smooth_kernel,... %rgb,
